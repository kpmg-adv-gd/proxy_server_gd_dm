const { getFilterPOD, getFilterPODTI, getFilterPODInstallation } = require("./library");
...
module.exports.listenerSetup = (app) => {
    ...
    app.post("/api/getFilterPODInstallation", async (req, res) => {
        ...
        try {
            ...
            var responseFilterData = await getFilterPODInstallation(plant, userId);
            res.status(200).json(responseFilterData);
        } catch (error) {
            let status = error.status || 500;
            let errMessage = error.message || "Internal Server Error";
