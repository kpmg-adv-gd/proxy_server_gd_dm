...
async function mangeConfirmationMarking(plant,personalNumber,wbe_machine,operation,mes_order,sfc,confirmation_number,marking_date,marked_labor,uom_marked_labor,variance_labor,uom_variance_labor,reason_for_variance,user_id,confirmation,cancellation,cancelled_confirmation,modification,workCenter,opDescription,project, defectId, phase){
    ...
    if (data.length == 0) { 
        ...
        // Testing
        await sendZDMConfirmationsTesting(plant, sfc, mes_order, wbe_machine, operation, personalNumber, data[0].network, data[0].activity_id, cancellation, "", "", confirmation_number, marking_date, (marked_labor + variance_labor).toString(), uom_marked_labor, reason_for_variance, "", "X", project, opDescription, user_id, modification, defectId, workCenter, phase);
    }
    if(cancellation=="X"){
