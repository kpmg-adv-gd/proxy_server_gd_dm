...
async function createOrder(plant, jsonOrderTesting) {
    ...
    var body = {
        ...
        "customValues": [
            ...
            { "attribute": "MANCANTI", "value": true},
            { "attribute": "COMPONENTI MANCANTI", "value": false},
...
async function saveZMarkingTesting(plant, jsonOrderTesting) {
    ...
    for (var i=0; i<jsonOrderTesting.level1.length; i++) {
        ...
        var res = await insertMarkingTesting(plant, jsonOrderTesting.wbs, level1.network, jsonOrderTesting.idOrdine, level1.idActivity, "00" + numberStep, level1.confirmationNumber, plannedLabor*100, "HCN", 0, level1.uomVariance, level1.areaRelevance);
        if (!res) result = false;
        numberStep += 10;
    }
    return result;
